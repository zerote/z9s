...
import (
	...
	"github.com/yourusername/z9s/buildinfo"
	"github.com/yourusername/z9s/health"

	"github.com/yourusername/z9s/k8s"
	"github.com/yourusername/z9s/metrics"
	"github.com/yourusername/z9s/ui"
)
...
func (app *Application) buildHeaderString(namespaceDisplay string) string {
	...
	// Format: Context: name | Cluster: version | User: user | Namespace: value | Metrics: status
	hdr.WriteString("[green]Context: [white]%s [green]| Cluster: [white]%s [green]| User: [white]%s [green]| Namespace: [white]%s [green]| Metrics:")
